...
const SkeletonCard: React.FC = () => (
  <div className="bg-[#0A0A0A] rounded-xl border border-[#1F1F1F] overflow-hidden animate-pulse min-w-[240px] flex-shrink-0">
...
const RecommendedCourses: React.FC<RecommendedCoursesProps> = ({ profileId }) => {
  ...
  if (isLoading) {
    return (
      <div>
        <h2 className="text-lg font-bold text-[#FAFAFA] mb-4">{t('recommendations.title')}</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {courses.map((course) => (
          ...
          >
            ...
            <div className="p-4 space-y-3">
              <h3 className="text-sm font-semibold text-[#FAFAFA] line-clamp-2 group-hover:text-white transition-colors">
                {course.title}
              </h3>
              <div className="flex items-center gap-2">
                <Avatar src={course.creator_avatar} name={course.creator_name} size="xs" />
                <span className="text-xs text-[#A0A0A0] truncate">{course.creator_name}</span>
              </div>
